...
class Ion_auth_model extends CI_Model
{
	...
	public function activate($id, $code = FALSE)
	{
		...
		if ($code !== FALSE)
		{
			$query = $this->db->select($this->username_column)
	 ...
	 * @author Mathew
	 */
	public function reset_password($username, $new) {
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['users'], $data, array($this->username_column => $username));
	...
	public function change_password($username, $old, $new)
	{
		...
		if ($old_password_matches === TRUE)
		{
			...
			$successfully_changed_password_in_db = $this->db->update($this->tables['users'], $data, array($this->username_column => $username));
	...
	public function username_check($username = '')
	{
		...
		return $this->db->where($this->username_column, $username)
						->limit(1)
						->count_all_results($this->tables['users']) > 0;
	}
	...
	public function forgotten_password($username)
	{
		...
		$key = $this->hash_code($activation_code_part . $username);
		...
		$this->db->update($this->tables['users'], $update, array($this->username_column => $username));
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$this->users_model->init_user($user);
	...
	public function login($username, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->username_column . ', email, id, password, active, last_login')
						  ->where($this->username_column, $username)
						  ->limit(1)
						  ->order_by('id', 'desc')
						  ->get($this->tables['users']);
		...
		if ($recheck !== 0)
		{
			...
			if ($last_login + $recheck < time())
			{
				$query = $this->db->select('id')
								  ->where(array($this->username_column => $this->session->userdata('username'), 'active' => '1'))
								  ->limit(1)
								  ->order_by('id', 'desc')
								  ->get($this->tables['users']);
				if ($query->num_rows() === 1)
				...
				else
				{
					...
					$username = $this->config->item('username', 'ion_auth');
	 ...
	 *                                Use get_last_attempt_ip($username) to retrieve a user's last IP
	 *
	 * @return boolean
	 */
	public function is_max_login_attempts_exceeded($username, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
			...
			if ($max_attempts > 0)
			{
				$attempts = $this->get_attempts_num($username, $ip_address);
	...
	/**
	 * Get number of login attempts for the given IP-address or username
	 * Based on code from Tank Auth, by Ilya Konyukhov (https://github.com/ilkon/Tank-Auth)
	 *
	 * @param string      $username   User's username
	 * @param string|null $ip_address IP address
	 ...
	 * @return int
	 */
	public function get_attempts_num($username, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
	 ...
	 */
	public function is_time_locked_out($username, $ip_address = NULL)
	{
		return $this->is_max_login_attempts_exceeded($username, $ip_address);
	}
	 ...
	 *                                Use get_last_attempt_ip($username) to retrieve a user's last IP
	 *
	 * @return int The time of the last login attempt for a given IP-address or username
	 */
	public function get_last_attempt_time($username, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
			$this->db->select('time');
	...
	/**
	 * Get the IP address of the last time a login attempt occured from given username
	 *
	 * @param string $username User's username
	 *
	 * @return string
	 */
	public function get_last_attempt_ip($username)
	{
	...
	public function increase_login_attempts($username)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
			$data = array('ip_address' => '', 'login' => $username, 'time' => time());
			if ($this->config->item('track_login_ip_address', 'ion_auth'))
			{
	 ...
	 * @return bool
	 */
	public function clear_login_attempts($username, $old_attempts_expire_period = 86400, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
	...
	public function update($id, array $data)
	{
		...
		if (array_key_exists($this->username_column, $data) && $this->username_check($data[$this->username_column]) && $user->{$this->username_column} !== $data[$this->username_column])
		{
		...
		if (array_key_exists($this->username_column, $data) || array_key_exists('password', $data) || array_key_exists('email', $data))
		{
	...
	public function set_session($user)
	{
		...
		$session_data = array(
		    'username'             => $user->{$this->username_column},
	...
	public function remember_user($id)
	{
		...
		if ($this->db->affected_rows() > -1)
		{
			...
			set_cookie(array(
			    'name'   => $this->config->item('username_cookie_name', 'ion_auth'),
			    'value'  => $user->{$this->username_column},
			    'expire' => $expire
			));
	...
	public function login_remembered_user()
	{
		...
		if (!get_cookie($this->config->item('username_cookie_name', 'ion_auth'))
			|| !get_cookie($this->config->item('remember_cookie_name', 'ion_auth'))
			|| !$this->username_check(get_cookie($this->config->item('username_cookie_name', 'ion_auth'))))
		{
		...
		// get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->username_column . ', id, email, last_login')
						  ->where($this->username_column, urldecode(get_cookie($this->config->item('username_cookie_name', 'ion_auth'))))
