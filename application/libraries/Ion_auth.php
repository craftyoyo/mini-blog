...
class Ion_auth
{
	...
	{
		if ($this->ion_auth_model->forgotten_password($username))
		{
			// Get user information
			$identifier = $this->ion_auth_model->username_column; // use model username column, so it can be overridden in a controller
			$user = $this->where($identifier, $username)->where('active', 1)->users()->row();
	...
	public function logout()
	{
		...
		{
			delete_cookie($this->config->item('username_cookie_name', 'ion_auth'));
		}
	...
	public function logged_in()
	{
		...
		if (!$recheck && get_cookie($this->config->item('username_cookie_name', 'ion_auth')) && get_cookie($this->config->item('remember_cookie_name', 'ion_auth')))
		{
